...
def setPositionDetailsAndCandles(pid, uid, symbol, row, cv, reportDate):
    ...
    dq = pd.DataFrame([row])
    dq.to_csv(
        f"F:\\AT\\report\\media\\{reportDate}\\positiondetails\\{pid}.csv",
        index=False)

    # setting position candles
    df = getterSpecificPastThirtyCandlesData(uid, symbol)
    df.to_csv(
        f"F:\\AT\\report\\media\\{reportDate}\\positioncandles\\{pid}.csv",
        index=False)

    # setting market structure df
    dm = getterMarketStructureDf()
    dm.to_csv(
        f"F:\\AT\\report\\media\\{reportDate}\\positionmdetails\\{pid}.csv",
        index=False)

    # setting market position candles
    df = getterSpecificPastThirtyCandlesData(120, "Nifty 100")
    df.to_csv(
        f"F:\\AT\\report\\media\\{reportDate}\\positionmcandles\\{pid}.csv",
        index=False)