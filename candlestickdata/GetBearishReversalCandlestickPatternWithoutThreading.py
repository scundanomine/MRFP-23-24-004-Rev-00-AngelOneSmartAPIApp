def getBearishReversalCandlestickPatternWithoutThreading(tdf):
    ...
    for r in range(10):
        ...
        if tdf["t"][r] == "shooting_star" and tdf.loc[r, 's'] >= 0.75:
            tdf.loc[r, "berRP"] = "shooting_star"
        # condition for double candlestick pattern
        elif r != 0:
            if tdf.loc[r - 1, "g"] == "green" and tdf.loc[r, "g"] == "red" and tdf.loc[r, 's'] >= 0.75:
                ...
                if tdf.loc[r - 1, "O"] > tdf.loc[r, "C"] and (tdf.loc[r, "O"] - tdf.loc[r - 1, "C"]) <= 0.25 * tdf.loc[r, "atr"]:
                    tdf.loc[r, "berRP"] = "Bearish_Engulfing"
            if r != 9 and tdf.loc[r+1, 's'] >= 0.75:
