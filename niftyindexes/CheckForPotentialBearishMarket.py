...
def checkBearishMarket():
    ...
    if df.loc[9, 'sma'] > cTwo:
        return True
    elif cTwo < cOne and checkBearishReversalPattern(df.loc[8, "bulRP"]) and df.loc[9, 'g'] == 'red' and df.loc[9, 'roc'] >= -15 and not checkBullishReversalCandle(df.loc[9, "t"]):
        return True
    elif df.loc[9, 'rsi0'] < df.loc[8, 'rsi1'] < df.loc[7, 'rsi2'] and (cTwo - cOne) <= -0.2*atr and df.loc[9, 'roc'] >= -15 and not checkBullishReversalCandle(df.loc[9, "t"]):
        return True
    else:
        return False
