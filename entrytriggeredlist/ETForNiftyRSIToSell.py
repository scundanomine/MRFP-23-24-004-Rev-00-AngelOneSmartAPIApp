...
def entryTriggeredForNiftyRSIToSell(lock):
    ...
    try:
        for index, row in rdf.iterrows():
            ...
            if bLDf['bFlag'][uid - 1] == 1 or cBLDf['bFlag'][uid - 1] == 1:
                ...
                atr = row['atr']
                # condition for sell
                if row['rsi0'] < row['rsi1'] < row['rsi2'] and (cTwo - cOne) <= -0.2*atr and not checkBullishReversalCandle(row["t"]):
                    # update the order type and upend the order list
                    row["ot"] = "sell"
                    row['oc'] = "ETFNiftyRSIDivergenceForSell"
                    row['srT'] = time.time()
                    with lock:
        ...
        print(f"Exception while entryTriggeredForNiftyRSIToSell: is {e}")
        time.sleep(1)
        entryTriggeredForNiftyRSIToSell(lock)


# entryTriggeredForNiftyRSIToSell()
