...
def entryTriggeredForSupportPivot(lock=multiprocessing.Lock()):
    ...
    for index, row in rdf.iterrows():
        ...
        if bLDf['bFlag'][uid-1] == 1:
            ...
            if cTwo <= rV and cOne <= rV and (cTwo - cOne) <= -0.25*atr and row['rsi0'] <= 40 and ("hammer" not in row["t"]):
            ...
            # condition for buy
            elif cTwo >= rV and cOne >= rV and (cTwo - cOne) >= 0.25*atr and checkBullishReversalPattern(row["bulRP"]) and ("shooting_star" not in row["t"]):
                # update the order type and upend the order list
                row["ot"] = "buy"
